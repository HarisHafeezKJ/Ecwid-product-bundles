...
import type { OfferLayout, ProductDivider, RuleType, WidgetStyle } from './types.js';
...
const TYPE_COPY_DEFAULTS: Record<RuleType, Partial<WidgetStyle>> = {
  VOLUME_DISCOUNT: {
...
function parseSize(value: unknown, fallback: number, min = 10, max = 32): number {
  return clampNumber(asNumber(value, fallback), min, max);
}

function parseRadius(value: unknown, fallback: number): number {
  return clampNumber(asNumber(value, fallback), 0, 32);
}

function parseProductDivider(value: unknown, fallback: ProductDivider): ProductDivider {
  if (value === 'LINE' || value === 'PLUS' || value === 'PLUS_LINE') return value;
  return fallback;
}

function parseLayout(value: unknown, fallback: OfferLayout): OfferLayout {
  return value === 'HORIZONTAL' ? 'HORIZONTAL' : fallback;
}

export function defaultWidgetStyle(ruleType?: RuleType): WidgetStyle {
  const base = { ...DEFAULT_WIDGET_STYLE };
  if (!ruleType) return base;
...
export function parseWidgetStyle(
  raw: unknown,
  ruleType?: RuleType,
): WidgetStyle {
  const defaults = defaultWidgetStyle(ruleType);
  const input = raw && typeof raw === 'object' ? (raw as Record<string, unknown>) : {};
  const style: WidgetStyle = { ...defaults };
  ...
  style.offerCardSelectedBg = parseHexColor(
    input.offerCardSelectedBg,
    defaults.offerCardSelectedBg ?? '#f0fdf4',
  );
  style.offerCardSelectedBorder = parseHexColor(
    input.offerCardSelectedBorder,
    defaults.offerCardSelectedBorder ?? '#22c55e',
  );
  ...
  style.offerSubtitleColor = parseHexColor(
    input.offerSubtitleColor,
    defaults.offerSubtitleColor ?? '#6b7280',
  );
  ...
  style.variationsWidth = clampNumber(
    asNumber(input.variationsWidth, defaults.variationsWidth ?? 100),
    20,
    100,
  );
  ...
  style.mixCardSelectedBorder = parseHexColor(
    input.mixCardSelectedBorder,
    defaults.mixCardSelectedBorder ?? '#22c55e',
  );
