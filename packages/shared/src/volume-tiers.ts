...
import type { TierDiscountType, VolumeTier, VolumeTiersWrap } from './types.js';
...
export function parseVolumeTiers(raw: unknown): VolumeTier[] {
  ...
  if (raw && typeof raw === 'object' && 'tiers' in raw) {
    return parseVolumeTiers((raw as VolumeTiersWrap).tiers);
  }

  return [];
}

function parseVolumeTier(raw: unknown): VolumeTier {
...
export function exactVolumeTier(tiers: VolumeTier[], qty: number): VolumeTier | undefined {
  const floorQty = Math.floor(qty);
  return tiers.find((tier) => tier.qty === floorQty && isTierDiscountable(tier));
}

export function bestVolumeTier(tiers: VolumeTier[], qty: number): VolumeTier | undefined {
  const floorQty = Math.floor(qty);
...
export function defaultVolumeTiersForRuleType(ruleType: 'VOLUME_DISCOUNT' | 'MIX_AND_MATCH'): VolumeTier[] {
...
export function mixRequiredCount(rule: { volumeTiers?: VolumeTiersWrap; requiredCount?: number }): number {
...
export function validateTierSequence(
  tiers: VolumeTier[],
  options: { exactQty?: boolean; minTiers?: number; maxTiers?: number } = {},
): string[] {
