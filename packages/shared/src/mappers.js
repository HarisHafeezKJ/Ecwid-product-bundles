...
function parseVolumeTiersWrap(raw) {
...
function parseStatus(value) {
    if (value === 'ACTIVE' || value === 'DRAFT' || value === 'DISABLED')
        return value;
    return 'ACTIVE';
}
function parsePlanTier(value) {
    if (value === 'FREE' || value === 'STARTER' || value === 'GROWTH' || value === 'PRO') {
        return value;
    }
    return 'FREE';
}
function inferApplyToAllProducts(row) {
    if (row.applyToAllProducts != null)
        return asBoolean(row.applyToAllProducts, true);
    const displayOn = parseDisplayOn(row.displayOn);
    const primaryProductId = asString(row.primaryProductId);
    if (displayOn === 'PRIMARY' && primaryProductId)
        return false;
    return true;
}
export function emptyRuleInput(ruleType, storeId) {
    ...
    return {
        ...
        items: { components: [] },
        volumeTiers: {
            tiers: ruleType === 'VOLUME_DISCOUNT' || ruleType === 'MIX_AND_MATCH'
        ...
        requiredCount: ruleType === 'MIX_AND_MATCH'
            ? defaultVolumeTiersForRuleType('MIX_AND_MATCH')[0]?.qty ?? 2
            : undefined,
        storeId,
    };
}
export function toBundleRule(raw, storeId) {
...
export function toAppSettings(raw, storeId) {
    return {
...
export function bundleRuleToInput(rule) {
    return {
        ...rule,
